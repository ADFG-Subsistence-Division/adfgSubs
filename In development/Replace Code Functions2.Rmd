...
findAndReplaceText2 <- function(
    ...
    if (!is.null(folders)) {
        files_from_folders <- unlist(lapply(folders, function(dir) {
    ...
    for (i in seq_along(files)) {
        ...
        res <- tryCatch({
            ...
            for (j in seq_along(find)) {
                # When fixed=FALSE, use perl=TRUE for multiline regex with (?s)
                file_content <- gsub(find[j], replace[j], file_content,
                                    fixed = fixed, perl = !fixed)
            }
            changed <- !identical(original_content, file_content)
            if (changed) {
                writeLines(strsplit(file_content, "\n")[[1]], file)
                if (verbose)
                    message(sprintf("Replaced text in: %s", file))
                results$status[i] <- "updated"
                results$message[i] <- "Text replaced"
            } else {
                if (verbose)
                    message(sprintf("No matching text found in: %s", file))
                results$status[i] <- "skipped"
                results$message[i] <- "No matching text"
            }
        }, error = function(e) {
